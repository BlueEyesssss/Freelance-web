...
<!--                        <div class="nav-search-input-2">
                            <div class="w-form">
                                <form action="MainController" id="email-form-2" name="email-form-2" data-name="Email Form 2" method="get"><input
                                        ...
                                        data-name="Name" placeholder="find Project" id="name" /><input type="submit" name="action" value="Search Job By Name"
                    ...
                    <div class="navigation-button-wrapper">
                        <div data-hover="false" data-delay="0" class="w-dropdown">
                            ...
                            <nav class="dropdown-list-2 w-dropdown-list"><a href="MainController?action=ViewSeekerProfile"
                                                                            class="dropdown-link w-dropdown-link">My Profile</a><a href="MainController?action=ViewBalanceHirerSeeker&role=seeker"
                                                                            class="dropdown-link w-dropdown-link">Balance</a><a href="MainController?action=Logout"
        ...
        <div class="container-2 w-container">
            <div class="my-proposal">My proposals</div>
        </div>
        <div class="container-3 w-container">
            <div class="invitations-wrapper">
                <div class="invitations">Invitations to interview (<%=listInvitationProposal.size()%>)</div>
                <%
                    if (listInvitationProposal != null) {
                        if (listInvitationProposal.size() > 0) {
                            for (ProposalDTO proposal : listInvitationProposal) {
                %>
                <div class="submit-1">
                    <div class="time-submit">Initiated <%= proposal.getCreatedDate()%></div>
                    <div class="project-submit"><a href="MainController?action=ProposalDetail&proposalID=<%= proposal.getProposalID() %>&projectID=<%= proposal.getProjectID()%>&ac=interview" class="job-submit"><%= proposal.getProjectName()%></a></div>
            ...
            <div class="active-wrapper">
                <div class="invitations">Submitted Proposals (<%=listSubmittedProposal.size()%>)</div>
                <%
                    if (listSubmittedProposal != null) {
                        if (listSubmittedProposal.size() > 0) {
                            for (ProposalDTO proposal : listSubmittedProposal) {
                %>
                <div class="submit-1">
                    <div class="time-submit">Initiated <%= proposal.getCreatedDate()%></div>
