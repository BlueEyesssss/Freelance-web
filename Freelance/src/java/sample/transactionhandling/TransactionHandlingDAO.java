...
public class TransactionHandlingDAO {
    ...
    public boolean updateDateDone(int transactionHandlingID) throws SQLException {
        boolean check = false;
        Connection con = null;
        PreparedStatement ptm = null;
        try {
            ...
            if (con != null) {
                ...
                String sqlQuery = "update TransactionHandling\n" +
"set dateDone = ?\n" +
"where transactionHandlingID = ?";
                
                ptm = con.prepareStatement(sqlQuery);
                ptm.setString(1, java.time.LocalDate.now().toString());
                ptm.setInt(2, transactionHandlingID);
                check = ptm.executeUpdate() > 0 ? true : false;
            }
        } catch (Exception e) {
            e.printStackTrace();
