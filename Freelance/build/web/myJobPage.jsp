<%-- 
    Document   : myJobPage
    Created on : Jun 8, 2022, 8:35:37 PM
    Author     : Phat
--%>
...
<body class="body">
    ...
    <div class="container-2 w-container">
        <div class="my-proposal">My Jobs</div>
    </div>
    <div class="container-3 w-container">
        <div class="invitations-wrapper">
            <div class="invitations">Active Job (<%= listJobStarted.size() %>)</div>
           
            <%      
                    
                    if (listJobStarted != null) {
                        if (listJobStarted.size() > 0) {
                            for (ProposalDTO proposal : listJobStarted) {
                            
                %>
            
            <div class="submit-1">
                <div class="time-submit">Initiated <%= proposal.getCreatedDate() %></div>
                <div class="project-submit"><a href="MainController?action=ViewActiveProject&proposalID=<%= proposal.getProposalID() %>&projectID=<%= proposal.getProjectID() %>" class="job-submit"><%= proposal.getProjectName()  %></a></div>
